...
class IocProcessor:
    ...
    def add_ioc(
        ...
        if general_honeypot_name is not None:
            ioc_record = self.ioc_repo.add_honeypot_to_ioc(
                general_honeypot_name, ioc_record
            )

        ioc_record = self._update_days_seen(ioc_record)
        ioc_record.scanner = ioc_record.scanner or (attack_type == SCANNER)
        ioc_record.payload_request = ioc_record.payload_request or (
            attack_type == PAYLOAD_REQUEST
        )
    ...
    def _merge_iocs(self, existing: IOC, new: IOC) -> IOC:
        ...
        existing.destination_ports = sorted(
            set(existing.destination_ports + new.destination_ports)
        )
