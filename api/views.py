...
def feeds(request, feed_type, attack_type, age, format_):
    ...
    resp_data = feeds_response(iocs_queryset, format_)
...
def feeds_pagination(request):
    ...
    resp_data = feeds_response(iocs, "json")
...
def get_queryset(request, feed_type, attack_type, age, format_):
    ...
    if age == "recent":
        ...
        ordering = params.get("ordering", "-last_seen")
        ...
        # order by the number of times seen (default)
        ordering = params.get("ordering", "-times_seen")
        iocs = IOC.objects.filter(**query_dict).order_by(ordering)[:5000]
...
def feeds_response(iocs, format_):
    license_text = (
...
@api_view([GET])
def general_honeypot_view(request):
