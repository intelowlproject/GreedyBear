...
def cowrie_session_view(request):
    ...
    Queries can be performed using either an IP address to find all sessions from that source, or a SHA-256 hash to find sessions containing a specific command sequence.
    ...
    if include_similar:
        ...
        related_sessions = CowrieSession.objects.filter(commands__cluster__in=clusters)
        sessions = (sessions | related_sessions).distinct().prefetch_related("source", "commands", "credentials")
    response_data = {
        "query": observable,
    }
