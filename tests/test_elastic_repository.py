...
class TestElasticRepository(CustomTestCase):
    ...
    def test_has_honeypot_been_hit_returns_true_when_hits_exist(
        self, mock_search_class, mock_get_time_window
    ):
        mock_search = Mock()
        ...
        result = self.repo.has_honeypot_been_hit(
            minutes_back_to_lookup=10, honeypot_name="test_honeypot"
        )
        self.assertTrue(result)
        mock_search.query.assert_called_once()
        mock_search.filter.assert_called_once_with(
            "term", **{"type.keyword": "test_honeypot"}
        )
        mock_search.count.assert_called_once()

    @patch("greedybear.cronjobs.repositories.elastic.get_time_window")
    @patch("greedybear.cronjobs.repositories.elastic.Search")
    def test_has_honeypot_been_hit_returns_false_when_no_hits(
        self, mock_search_class, mock_get_time_window
    ):
        mock_search = Mock()
        mock_search_class.return_value = mock_search
    ...
    @patch("greedybear.cronjobs.repositories.elastic.get_time_window")
    @patch("greedybear.cronjobs.repositories.elastic.Search")
    def test_search_yields_all_hits_across_chunks(
        self, mock_search_class, mock_get_time_window
    ):
        mock_search = Mock()
        mock_search_class.return_value = mock_search
    ...
    @patch("greedybear.cronjobs.repositories.elastic.get_time_window")
    @patch("greedybear.cronjobs.repositories.elastic.Search")
    def test_search_returns_ordered_hits_within_chunks(
        self, mock_search_class, mock_get_time_window
    ):
        mock_search = Mock()
        mock_search_class.return_value = mock_search
...
class TestSearchChunking(CustomTestCase):
    ...
    @patch("greedybear.cronjobs.repositories.elastic.Search")
    def test_chunk_boundaries_are_correct(
        self, mock_search_class, mock_q, mock_get_time_window
    ):
        ...
        expected_calls = [
            call(
                "range",
                **{"@timestamp": {"gte": start, "lt": start + timedelta(minutes=10)}},
            ),
            call(
                "range",
                **{
                    "@timestamp": {
                        "gte": start + timedelta(minutes=10),
                        "lt": start + timedelta(minutes=20),
                    }
                },
            ),
            call(
                "range",
                **{"@timestamp": {"gte": start + timedelta(minutes=20), "lt": end}},
            ),
        ]
    ...
    @patch("greedybear.cronjobs.repositories.elastic.get_time_window")
    @patch("greedybear.cronjobs.repositories.elastic.Search")
    def test_equal_start_end_yields_no_chunks(
        self, mock_search_class, mock_get_time_window
    ):
    ...
    @patch("greedybear.cronjobs.repositories.elastic.get_time_window")
    @patch("greedybear.cronjobs.repositories.elastic.Search")
    def test_healthcheck_called_once_for_multiple_chunks(
        self, mock_search_class, mock_get_time_window
    ):
    ...
    @patch("greedybear.cronjobs.repositories.elastic.Q")
    @patch("greedybear.cronjobs.repositories.elastic.Search")
    def test_last_chunk_shorter_when_not_divisible(
        self, mock_search_class, mock_q, mock_get_time_window
    ):
...
class TestTimeWindowCalculation(CustomTestCase):
    def test_basic_10min_window(self):
        ...
        start, end = get_time_window(
            reference, lookback_minutes=10, extraction_interval=10
        )
    ...
    def test_with_custom_lookback(self):
        ...
        reference = datetime(2024, 1, 10, 14, 23)  # 14:23
        start, end = get_time_window(
            reference, lookback_minutes=15, extraction_interval=10
        )
    ...
    def test_with_custom_extraction_interval(self):
        ...
        reference = datetime(2024, 1, 10, 14, 23)  # 14:23
        start, end = get_time_window(
            reference, lookback_minutes=15, extraction_interval=15
        )
    ...
    def test_large_lookback(self):
        ...
        reference = datetime(2024, 1, 10, 14, 23)  # 14:23
        start, end = get_time_window(
            reference, lookback_minutes=60 * 24 * 3, extraction_interval=10
        )
